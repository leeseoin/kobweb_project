      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-black text-slate-800 mb-2">이력서 만들기</h1>
              <p className="text-slate-700 font-semibold">단계별로 완성하는 나만의 전문 이력서</p>
            </div>
            <div className="flex items-center space-x-3">
              <button
                onClick={handleSaveDraft}
                disabled={isSaving}
                className="flex items-center space-x-2 bg-white hover:bg-slate-100 text-slate-700 border-2 border-slate-400 px-4 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap disabled:opacity-50 font-semibold shadow-sm hover:shadow-md"
              >
                {isSaving ? (
                  <>
              ...
              <Link
                href="/"
                className="flex items-center space-x-2 bg-slate-200 hover:bg-slate-300 text-slate-800 px-4 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap font-semibold border-2 border-slate-400 shadow-sm hover:shadow-md"
              >
        ...
        <div className="flex gap-8">
          ...
          <div className="w-80 flex-shrink-0">
            <div className="bg-white border-2 border-slate-300 rounded-xl p-6 sticky top-6 shadow-lg">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-black text-slate-800">진행 상황</h3>
                <button
                  onClick={handleSaveDraft}
                  disabled={isSaving}
                  className="text-slate-700 hover:text-blue-700 transition-colors disabled:opacity-50"
                  title="임시저장"
                >
                  {isSaving ? (
              ...
              <div className="space-y-4">
                {steps.map((step, index) => (
                  <div
                    ...
                    className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer transition-all font-semibold ${
              ...
              <div className="mt-8 pt-6 border-t-2 border-slate-300">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-slate-700 font-semibold">전체 진행률</span>
                  <span className="text-sm font-black text-slate-800">
                    {Math.round((currentStep / steps.length) * 100)}%
                  </span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3 border-2 border-slate-300">
                  <div
                    className="bg-gradient-to-r from-blue-600 to-cyan-600 h-full rounded-full transition-all duration-300 shadow-md"
