...
export function useCurrentUser() {
  const result = useApi(() => apiClient.getCurrentUser());
  return result;
}
...
export function useCreateContact() {
  ...
  const createContact = async (contact: Omit<Contact, 'businessCardId'>) => {
    try {
      ...
      console.log('📤 명함 등록 요청 데이터:', businessCardData);
      const response = await apiClient.createBusinessCard(businessCardData);
      console.log('📥 명함 등록 응답:', response);
...
export function useAcceptBusinessCardRequest() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const acceptRequest = async (businessCardId: string) => {
    try {
      setLoading(true);
      setError(null);
      await apiClient.acceptBusinessCardRequest(businessCardId);

      // Invalidate contacts and business cards list after successful acceptance
      invalidateQueries('contacts');
      invalidateQueries('business-cards');

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to accept business card request');
      throw err;
    } finally {
      setLoading(false);
    }
...
export function useRejectBusinessCardRequest() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const rejectRequest = async (businessCardId: string) => {
    try {
      setLoading(true);
      setError(null);
      await apiClient.rejectBusinessCardRequest(businessCardId);

      // Invalidate contacts and business cards list after rejection
      invalidateQueries('contacts');
      invalidateQueries('business-cards');

    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to reject business card request');
      throw err;
    } finally {
      setLoading(false);
    }
