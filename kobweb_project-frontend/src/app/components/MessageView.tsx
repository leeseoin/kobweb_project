...
export default function MessageView() {
  ...
  // WebSocket connection - 싱글톤 인스턴스 사용
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 flex-1 min-w-0">
                      <h3 className="text-sm font-medium text-[#1E2022] truncate">
                        {getDisplayName(room)}
                      </h3>
                      {/* 안 읽은 메시지 카운트 배지 */}
                      {room.unreadCount && room.unreadCount > 0 && (
                        <span className="flex-shrink-0 min-w-[20px] h-5 px-2 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
                          {room.unreadCount > 99 ? '99+' : room.unreadCount}
                        </span>
                      )}
                    </div>
                    <span className="text-xs text-[#52616B] ml-2 flex-shrink-0">{room.lastMessage ? formatTime(room.lastMessage.sentAt) : ''}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <p className={`text-sm truncate ${room.unreadCount && room.unreadCount > 0 ? 'text-[#1E2022] font-semibold' : 'text-[#52616B]'}`}>
