...
**2️⃣ WebSocket 싱글톤 패턴 구현 (프론트엔드)**

**문제**
- MessageView와 WebSocketTest가 각각 `useChatWebSocket()`을 호출하여 중복 연결 생성
- 여러 개의 WebSocket 세션이 동시에 생성됨

**해결**
- Context API를 사용한 싱글톤 패턴 구현

**새로 생성된 파일**
- `WebSocketContext.tsx` - WebSocket 싱글톤 Context Provider

**변경된 파일**
1. `layout.tsx` - WebSocketProvider로 앱 전체 감싸기
2. `MessageView.tsx`
   - `useChatWebSocket()` → `useWebSocketContext()` 변경
   - 불필요한 WebSocketTest import 제거
3. `WebSocketTest.tsx` - `useWebSocketContext()` 사용

**구조**
```
App Root (layout.tsx)
  └── <WebSocketProvider>  ← 여기서 단 한 번만 WebSocket 연결 생성
       └── MessageView, WebSocketTest 등이 동일한 인스턴스 공유
```
...
이제 Signup 페이지와 Login 페이지의 디자인이 완벽하게 일치합니다:
