...
import { useChatRooms, useMessages, useSendMessage, useCreateChatRoom, useContacts, useCurrentUser } from '../hooks/useApi';
...
export default function MessageView() {
  ...
  useEffect(() => {
    if (selectedRoomId && webSocket.isConnected) {
      ...
      // 채팅방 정보 요청
      // webSocket.subscribeToRoomInfo(selectedRoomId); // Backend doesn't support /app/room-info endpoint

      // 실시간 메시지 초기화 - 이전 방의 메시지 제거
      setRealTimeMessages(prev => prev.filter(msg => msg.roomId === selectedRoomId));
    }
  }, [selectedRoomId, webSocket.isConnected, webSocket.subscribeToRoom]);
    ...
    try {
      ...
      if (webSocket.isConnected && webSocket.connectionStatus === 'connected') {
        ...
        if (webSocket.connectionStatus === 'connecting') {
          ...
          // 다시 확인
          if (webSocket.isConnected && (webSocket.connectionStatus as string) === 'connected') {
            const success = webSocket.sendChatMessage(selectedRoomId, messageContent, clientMessageId);
            if (success) {
      ...
      // 임시 메시지를 실시간 메시지에 추가 (낙관적 업데이트)
      const tempMessage = {
        messageId: clientMessageId,
        roomId: selectedRoomId,
        content: messageContent,
        sender: {
          id: currentUserId, // 현재 사용자의 실제 ID 사용
          nickname: '나'
        },
        sentAt: new Date().toISOString(),
        sequence: Date.now()
      ...
      // 백엔드 저장을 위해 약간의 지연 후 메시지 다시 가져오기
      setTimeout(() => {
        refetchMessages();
      }, 500);
  ...
  const displayMessages = useMemo(() => {
    ...
    const uniqueMessages = standardizedMessages.filter((message, index, self) => {
      return index === self.findIndex(m => {
        ...
        // content, sender.id가 같고 시간 차이가 2초 이내면 중복으로 판단
              ...
              <div className="flex items-center space-x-3">
                <div className="relative">
                  {room.type === 'DIRECT' ? (
                    // 1:1 대화방: 상대방 프로필 이미지
                    <Avatar
                      src={(room.participants.find((p: any) => p.id !== currentUserId) as any)?.avatar}
              ...
              <div className="flex items-center space-x-3">
                <div className="relative">
                  {selectedRoom.type === 'DIRECT' ? (
                    <Avatar
                      src={(selectedRoom.participants.find((p: any) => p.id !== currentUserId) as any)?.avatar}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              {contacts && contacts.filter(contact => contact.email !== currentUser?.email).length > 0 ? (
                contacts.filter(contact => contact.email !== currentUser?.email).map((contact, index) => (
                  <div
                    key={contact.businessCardId || contact.id || `contact-${index}`}
