...
export default function ContactDetail({ contact, onContactUpdated }: ContactDetailProps) {
  ...
  if (!contact) {
    return (
      <div className="bg-white border border-[#bfc7d1] rounded-lg h-full flex flex-col items-center justify-center text-center p-12">
        <div className="w-20 h-20 bg-[#e1e4e6] rounded-full flex items-center justify-center mb-4">
          <i className="ri-user-line w-10 h-10 text-[#52616B] flex items-center justify-center"></i>
        </div>
        <h3 className="text-[#1E2022] font-medium mb-2">연락처를 선택해주세요</h3>
        <p className="text-[#52616B] text-sm max-w-xs">왼쪽 목록에서 연락처를 선택하면 상세 정보를 확인할 수 있습니다.</p>
      </div>
    );
  }
  ...
  if (isEditing) {
    return (
      <div className="bg-white border border-[#bfc7d1] rounded-lg h-full">
        <BusinessCardEditor
  ...
  return (
    <div className="bg-white border border-[#bfc7d1] rounded-lg p-6 h-full">
      <div className="flex items-start space-x-4 mb-6">
        <div className="w-20 h-20 rounded-full overflow-hidden bg-[#bfc7d1] flex-shrink-0 flex items-center justify-center">
          {contact.avatar ? (
            <img
              src={contact.avatar}
              alt={contact.name}
