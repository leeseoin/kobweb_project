...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, UUID> {
    ...
    // 채팅방의 현재 sequence 조회
    @Query("SELECT cr.nextSequence FROM ChatRoom cr WHERE cr.id = :roomId")
    Long getCurrentSequence(@Param("roomId") UUID roomId);

    // 채팅방의 sequence 증가 (Optimistic Locking으로 동시성 제어)
    @Modifying
    @Query("UPDATE ChatRoom cr SET cr.nextSequence = cr.nextSequence + 1 WHERE cr.id = :roomId")
    int incrementSequence(@Param("roomId") UUID roomId);
} 